<template>
	<view @touchmove.stop.prevent>
		<view class="fui-fab-box" :class="{'fui-fab-right':!left || (left && right)}" :style="{left:getLeft(),right:getRight(),bottom:bottom+'rpx'}">
			<view class="fui-fab-btn" :class="{'fui-visible':isOpen || isShow,'fui-fab-hidden':hidden && !isShow}">
			...
			<view v-if="!isShow" class="fui-fab-item" :class="{'fui-active':isOpen}" :style="{width:width+'rpx',height:height+'rpx',borderRadius:radius,background:bgColor,color:color}"
...
<script>
	...
	export default {
		...
		props: {
			...
			// bgColor: "#5677fc",
			// //图标/图片地址
			// imgUrl: "https://cnd.dzwztea.cn/tea/images/fab/fab_reward.png",
			// //图片高度 rpx
			// imgHeight: 60,
			// //图片宽度 rpx
			// imgWidth: 60,
