	...
	<view>
		<view :class="{ 'fui-bottom-popup': isFixed, 'fui-popup-show': isShow && isFixed }">
			...
			<view class="fui-date-box">
				<view class="fui-iconfont fui-font-arrowleft" :style="{ color: yearArrowColor }" hover-class="fui-opacity"
				 :hover-stay-time="150" v-if="arrowType == 1" @tap="changeYear(0)"></view>
				<view class="fui-iconfont fui-font-arrowleft" :style="{ color: monthArrowColor }" hover-class="fui-opacity"
				 :hover-stay-time="150" @tap="changeMonth(0)"></view>
				<view class="fui-date_time">{{ showTitle }}</view>
				<view class="fui-iconfont fui-font-arrowright" :style="{ color: monthArrowColor }" hover-class="fui-opacity"
				 :hover-stay-time="150" @tap="changeMonth(1)"></view>
				<view class="fui-iconfont fui-font-arrowright" :style="{ color: yearArrowColor }" hover-class="fui-opacity"
			...
			<view class="fui-date-content" :class="{ 'fui-flex-start': isFixed && fixedHeight }" :style="{ height: isFixed && fixedHeight ? dateHeight * 6 + 'px' : 'auto' }">
				<block v-for="(item, index) in weekdayArr" :key="index">
					<view class="fui-date"></view>
				</block>
				<view class="fui-date" :class="{
				 ...
				 v-for="(item, index) in daysArr" :key="index" @tap="dateClick(index)">
					<view class="fui-date-text" :style="{ color: isFixed ? getColor(index, 2) : getStatusData(3, index), backgroundColor: getStatusData(2, index) }">
						<view v-if="isFixed || !getStatusData(4, index)">{{ index + 1 }}</view>
						<view v-if="!getStatusData(4, index)" class="fui-custom-desc" :class="{'fui-lunar-unshow':!lunar && isFixed}">{{ getDescText(index,startDate,endDate)}}</view>
						<text class="fui-iconfont fui-font-check" v-if="getStatusData(4, index)"></text>
					</view>
					<view class="fui-date-desc" :style="{ color: activeColor }" v-if="!lunar && type == 2 && startDate == `${year}-${month}-${index + 1}` && startDate != endDate">
						{{ startText }}
					</view>
					<view class="fui-date-desc" :style="{ color: activeColor }" v-if="!lunar && type == 2 && endDate == `${year}-${month}-${index + 1}`">{{ endText }}</view>
				</view>
		...
		<view class="fui-popup-mask" :class="[isShow ? 'fui-mask-show' : '']" @touchmove.stop.prevent="stop" v-if="isFixed"
		 @tap="hide"></view>
	</view>
</template>
<script>
	...
	const calendar = require("./fui-calendar.js")
	export default {
		name: 'fuiCalendar',
		// components:{
		// 	fuiButton
