<?php

use yii\db\Migration;

class m231104_123104_bea_cloud_withdraw_log extends Migration
{
    public function up()
    {
        /* 取消外键约束 */
        $this->execute('SET foreign_key_checks = 0');
        
        /* 创建表 */
        $this->createTable('{{%bea_cloud_withdraw_log}}', [
            'id' => "int(11) NOT NULL AUTO_INCREMENT",
            'bloc_id' => "int(11) NULL",
            'store_id' => "int(11) NULL",
            'create_time' => "int(11) NULL COMMENT '申请时间'",
            'update_time' => "int(11) NULL",
            'partner_trade_no' => "varchar(30) NULL COMMENT '付款商户单号'",
            'money' => "decimal(11,2) NULL COMMENT '到账金额'",
            'money_count' => "decimal(11,2) NOT NULL COMMENT '申请金额'",
            'withdraw_type' => "int(11) NULL COMMENT '提现类型1.余额'",
            'withdraw_status' => "int(11) NULL COMMENT '提现状态1.申请中  2.已完成 3.已驳回'",
            'member_id' => "int(11) NULL COMMENT '用户id'",
            'pay_status' => "int(11) NULL COMMENT '支付状态1.已支付2.未支付'",
            'pay_type' => "int(11) NULL COMMENT '支付方式1.现金'",
            'confirm_name' => "varchar(30) NULL COMMENT '确认人'",
            'openid' => "varchar(30) NULL COMMENT 'OPENID'",
            're_user_name' => "varchar(30) NULL COMMENT '真实姓名'",
            'desc' => "varchar(255) NULL COMMENT '付款说明'",
            'service_charge' => "decimal(11,2) NULL COMMENT '手续费'",
            'mobile' => "varchar(30) NULL COMMENT '手机号'",
            'PRIMARY KEY (`id`)'
        ], "ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci ROW_FORMAT=DYNAMIC COMMENT='申请提现日志'");
        
        /* 索引设置 */
        
        
        /* 表数据 */
        
        /* 设置外键约束 */
        $this->execute('SET foreign_key_checks = 1;');
    }

    public function down()
    {
        $this->execute('SET foreign_key_checks = 0');
        /* 删除表 */
        $this->dropTable('{{%bea_cloud_withdraw_log}}');
        $this->execute('SET foreign_key_checks = 1;');
    }
}

