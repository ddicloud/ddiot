 ...
 * @Last Modified time: 2022-07-18 17:09:33
 */
...
class GlobalsService extends BaseService
{
    ...
    public function getConf($bloc_id = 0)
    {
        ...
        if (empty($conf['api'])) {
            // api签名验证
            $apiConf->getConf($global_bloc_id);
            $conf['api'] = $apiConf;
        }
        ...
        if (empty($conf['api'])) {
            // api签名验证
            $conf['api'] = Yii::$app->settings->getAllBySection('Api');
        }
    ...
    public function getBlocLevel()
    {
        ...
        $key = ['getBlocLevel', $global_bloc_id];
