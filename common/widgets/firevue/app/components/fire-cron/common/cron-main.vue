...
<script>
  ...
  import dateFormat from '../common/format-date'
    ...
    computed: {
      tableData() {
        ...
        c = c.concat([
          ...
          {name: '天', value: this.day},
          {name: '月', value: this.month},
          {name: '周', value: this.week}
        ])
        // console.log('call in tableData')
        // console.log(c)
        return (this.hideSecond || this.hideYear) ? c.concat({name: '定时格式:', value: this.cronValue_c})
          : c.concat(
            {name: '年', value: this.year},
            {name: '定时格式:', value: this.cronValue_c},
            {name: '定时格式(Exclusive Year):', value: this.cronValue_c2}
          )
      },
      cronValue_c() {
        let result = []
